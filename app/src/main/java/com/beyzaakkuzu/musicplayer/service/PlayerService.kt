...
import com.beyzaakkuzu.musicplayer.receivers.NotificationActionBroadcast

import com.beyzaakkuzu.musicplayer.util.*
import com.beyzaakkuzu.musicplayer.util.Constants.CURRENT_SONG_DURATION_KEY
import com.beyzaakkuzu.musicplayer.util.Constants.NOTIFICATION_CHANNEL_ID
import com.beyzaakkuzu.musicplayer.util.Constants.NOTIFICATION_CHANNEL_NAME
import com.beyzaakkuzu.musicplayer.util.Constants.NOTIFICATION_ID
import com.beyzaakkuzu.musicplayer.util.Constants.POSITION_KEY
...
class PlayerService : Service(), MediaPlayer.OnCompletionListener, MediaPlayer.OnPreparedListener,
    ...
    private var originalSongList: List<SongModel> = ArrayList()
    private val iBinder: IBinder = LocalBinder()

    var mediaPlayer: MediaPlayer? = null
    var currentSong: SongModel? = null
    private var listOfAllSong: List<SongModel> = ArrayList()

    private val songFromSharedPreferences: SongModel?
        get() = SharedPreference.getCurrentSong(sharedPreferences)
    ...
    override fun onCreate() {
        ...
        if (currentSong == null) {
            currentSong = SharedPreference.getCurrentSong(sharedPreferences)
        }
    }
        ...
        when (focusState) {
            ...
            AudioManager.AUDIOFOCUS_LOSS -> {
                if (mediaPlayer != null && isPlaying()) {
                    SharedPreference.saveCurrentPosition(
                        sharedPreferences,
                        getCurrentPosition()
                    )
                    mediaPlayer?.stop()
    ...
    override fun onDestroy() {
        ...
        if (mediaPlayer != null) {
            ...
            SharedPreference.saveCurrentPosition(sharedPreferences, getCurrentPosition())
            mediaPlayer?.stop()
            notifyPlayPauseStateChanged()
            mediaPlayer?.release()
        }
    ...
    fun getAllSongs(songList: List<SongModel>?, clickedPosition: Int) {
    ...
    private fun initMediaPlayer(position: Int) {
        ...
        SharedPreference.saveCurrentSong(currentSong!!, sharedPreferences)
    ...
    private fun getNextPosition(): Int {
        ...
        if (position == -1) {
            nextPosition = SharedPreference.getPosition(sharedPreferences) + 1
        }
    ...
    private fun getPreviousPosition(): Int {
        ...
        if (position == -1) {
            prePosition = SharedPreference.getPosition(sharedPreferences) - 1
        }
    ...
    private fun startForegroundService() {
        ...
        val previousIntent = Intent(this, NotificationActionBroadcast::class.java).also {
        ...
        val playPauseIntent = Intent(this, NotificationActionBroadcast::class.java).also {
        ...
        val nextIntent = Intent(this, NotificationActionBroadcast::class.java).also {
